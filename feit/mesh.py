...
class Electrodes:
    ...
    def __init__(
        self, num_electrodes, coverage_fraction, rotation_angle=0, *, anticlockwise=True
    ):
        if not isinstance(num_electrodes, int):
            raise TypeError("`num_electrodes` must be an int.")
    ...
    def __calc_position(self):
        ...
        """
        size_e = (
            2 * np.pi / self.num_electrodes * self.coverage_fraction
        )  # Size electrodes
        size_gap = (
            2 * np.pi / self.num_electrodes * (1 - self.coverage_fraction)
        )  # Size gaps
...
def generate_electrodes_mesh(
    ...
    num_elec_vertices : int
        Number of vertices in each electrode.
    num_gap_vertices : int
    ...
    if not isinstance(num_elec_vertices, int):
        raise TypeError("`num_elec_vertices` must be an int.")
    if not isinstance(num_gap_vertices, int):
        raise TypeError("`num_gap_vertices` must be an int.")
    ...
    for i in range(num_electrodes):
        ...
        for theta in np.linspace(theta0, thetai, num_elec_vertices):
            pt = [
        ...
        for theta in np.linspace(theta0, thetai, num_gap_vertices + 2):
